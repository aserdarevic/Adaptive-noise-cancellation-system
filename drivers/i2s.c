...
// Configure the I2S module 
  ...
  FM4_I2S0->CNTREG_f.CKRT  = 0u;       // 0: Bypass: Use Wolfson clock
	//Overhead bits
  if (sampling_rate == FS_8000_HZ) FM4_I2S0->CNTREG_f.OVHD  = 352;     
  if (sampling_rate == FS_32000_HZ) FM4_I2S0->CNTREG_f.OVHD  = 64;     
  if (sampling_rate == FS_48000_HZ) FM4_I2S0->CNTREG_f.OVHD  = 32;     
  if (sampling_rate == FS_96000_HZ) FM4_I2S0->CNTREG_f.OVHD  = 0;     
  ...
  if (mode == IO_METHOD_INTR)FM4_I2S0->INTCNT_f.TXFDM  = TRUE;
	else if (mode == IO_METHOD_DMA)FM4_I2S0->INTCNT_f.TXFDM  = FALSE; //A DMA request is made when the receive dat written to the transmit FIFO meets or exceeds the threshold value.
  if (mode == IO_METHOD_INTR) FM4_I2S0->INTCNT_f.TXFIM  = FALSE;    //An interrupt is issued to the CPU when the transmit FIFO empty space meets or exceeds the threshold value
	else if (mode == IO_METHOD_DMA)FM4_I2S0->INTCNT_f.TXFIM  = TRUE;
  ...
  if (mode == IO_METHOD_INTR)FM4_I2S0->INTCNT_f.RXFDM  = TRUE;
	else if (mode == IO_METHOD_DMA)FM4_I2S0->INTCNT_f.RXFDM  = FALSE; //A DMA request is made when the receive data written to the transmit FIFO meets or exceeds the threshold value.
...
I2S_Data_TypeDef I2S_Data;

__weak void ProcessData(I2S_Data_TypeDef* I2S_Data)
{
}
...
void PRGCRC_I2S_IRQHandler(void)
{
    ...
    audio_IN = i2s_rx();
    I2S_Data.input_l = (int16_t)(audio_IN & 0x0000FFFF);       //Separate 16 bits channel left
    I2S_Data.input_r = (int16_t)((audio_IN >> 16)& 0x0000FFFF); //Separate 16 bits channel right

    //Process
    I2S_Data.mask = AUDIO_CH_BOTH;
    ProcessData(&I2S_Data);
    
    audio_OUT = ((I2S_Data.output_r << 16 & 0xFFFF0000)) + (I2S_Data.output_l & 0x0000FFFF);	//Put the two channels toguether again
    i2s_tx(audio_OUT);
