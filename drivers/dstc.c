...
//Using DMA
int16_t dma_tx_buffer[2][DMA_BUFFER_SIZE][2],
        dma_rx_buffer[2][DMA_BUFFER_SIZE][2];

//volatile uint32_t rx_proc_buffer = PING;
//volatile uint32_t tx_proc_buffer = PING;
int16_t (*tx_proc_buffer)[2] = dma_tx_buffer[PING];
int16_t (*rx_proc_buffer)[2] = dma_rx_buffer[PING];
volatile uint32_t tx_buffer_empty = 0;
volatile uint32_t rx_buffer_full = 0;
...
void dstc_init(void) {   
  ...
  // DES2
  stcDES[0u].DES2 = (uint32_t)&dma_tx_buffer[PING];   // Source address (incremented by TW * 1 for every transfer. Configured in DES0.SAC)
  ...
  // DES3
  stcDES[1u].DES3 = (uint32_t)&dma_rx_buffer[PING];    // Destination address - I2S Transmission data register (Same for every transfer. Configured in DES0.DAC)
	// DES4
...
{
    /* Tx Channel Interrupt */
    if (dstc_state(0))
    {
        if (tx_proc_buffer == dma_tx_buffer[PONG])
        {
            dstc_src_memory(0, (uint32_t)&(dma_tx_buffer[PONG]));    //Soucrce address
            tx_proc_buffer = dma_tx_buffer[PING];
        }
        else
        {
            dstc_src_memory(0, (uint32_t)&(dma_tx_buffer[PING]));    //Soucrce address
            tx_proc_buffer = dma_tx_buffer[PONG];
        }
    ...
    /* Rx Channel Interrupt */
    if (dstc_state(1))
    {
        if (rx_proc_buffer == dma_rx_buffer[PONG])
        {
            dstc_dest_memory(1, (uint32_t)&(dma_rx_buffer[PONG]));   //Destination address
            rx_proc_buffer = dma_rx_buffer[PING];
        }
        else
        {
            dstc_dest_memory(1, (uint32_t)&(dma_rx_buffer[PING]));   //Destination address
            rx_proc_buffer = dma_rx_buffer[PONG];
        }
        rx_buffer_full = 1;
